...
const Partners = () => {
  ...
  // 1. Import all partner images
  const partnerImages = {};
  for (let i = 1; i <= 41; i++) {
    partnerImages[i] = import(`@/assets/partners/${i}.jpg`);
  }

  // 2. Create partners list with actual imports
  const partners = Array.from({ length: 41 }, (_, i) => ({
    id: i + 1,
    name: `شريك ${i + 1}`,
    logo: partnerImages[i + 1], // This will be a promise initially
  }));
