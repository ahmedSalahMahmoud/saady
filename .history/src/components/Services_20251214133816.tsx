...
const Services = () => {
  ...
  const services = [
    ...
    },
    {
      icon: Wrench,
      title: 'الكهروميكانيك',
      description: 'صيانة وتشغيل الأنظمة الكهربائية والميكانيكية بكفاءة عالية',
    },
    {
      icon: Snowflake,
      title: 'التبريد والتكييف',
      description: 'تركيب وصيانة أنظمة التكييف المركزي والتبريد للمباني التجارية والسكنية',
    },
    {
      icon: Hammer,
      title: 'تأهيل وترميم المباني',
      description: 'إعادة تأهيل المباني القديمة وترميمها وفق أحدث التقنيات والمواصفات',
    },
    {
      icon: SparklesIcon,
      title: 'خدمات النظافة',
      description: 'خدمات نظافة متكاملة للمنشآت والمباني التجارية والسكنية',
    },
  ];
  ...
  const handleScroll = () => {
    if (scrollRef.current) {
      ...
      const newIndex = Math.round(scrollPosition / (itemWidth * 0.4));
  ...
  const scrollToIndex = (index: number) => {
    if (scrollRef.current) {
      const itemWidth = scrollRef.current.offsetWidth * 0.4;
      ...
      <div className="w-full overflow-x-auto scrollbar-hide" style={{ scrollbarWidth: 'none', msOverflowStyle: 'none' }}>
        ...
        >
          {services.map((service, index) => (
            <div
              key={index}
              className={`flex-shrink-0 w-[85vw] md:w-[40vw] lg:w-[30vw] group transition-all duration-300 ${
