...
const Partners = () => {
  ...
  // Create 8 copies (40 total) for seamless RTL loop
  const partnerCopies = [
    ...realPartners,
    ...realPartners,
    ...realPartners,
  ...
  return (
    <section id="partners" className="py-20 lg:py-32 bg-muted/30" dir="rtl">
      <div className="container">
        ...
        {/* Partners Slider - RTL Continuous Animation */}
        <div
          className="w-full overflow-hidden relative"
          onMouseEnter={() => setIsPaused(true)}
          onMouseLeave={() => setIsPaused(false)}
        >
          {/* Right gradient overlay (for RTL start) */}
          ...
          {/* Left gradient overlay (for RTL end) */}
          <div className="absolute left-0 top-0 bottom-0 w-32 bg-gradient-to-r from-muted/30 to-transparent z-10 pointer-events-none" />

          <div
            className="flex pb-8"
            style={{
              animation: isPaused ? 'none' : 'scrollLTR 30s linear infinite',
              width: 'max-content',
              gap: '4rem',
              direction: 'ltr' // Important for animation direction
            }}
          >
        ...
        <style jsx>{`
          /* RTL Animation: Moves from RIGHT to LEFT */
          @keyframes scrollLTR {
            0% {
              transform: translateX(0);
            }
            100% {
              transform: translateX(-12.5%); /* Moves left */
            }
          }
        `}</style>
      </div>
